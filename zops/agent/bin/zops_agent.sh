...
process_list=(ops_agent/agent.py)
...
function start(){
    ...
    do
        ...
        nohup python ${proc} > /var/log/zops_agent 2>&1 &
        daemonpid=$!
        sleep 1
        if kill -0 $daemonpid ; then
            ...
            echo -e "start zops_agent\t\t[\033[0;31;1mFAILED\033[0m]"
            return 1
        fi
    done
    touch /var/lock/subsys/zops_agent
    echo -e "start zops_agent\t\t[\033[0;32;1mOK\033[0m]"
}
...
function stop(){
    ...
    rm -f /var/lock/subsys/zops_agent
    echo -e "stop zops_agent [\033[0;32;1mOK\033[0m]"
}
