...
class ReqServer(object):
    ...
    def do(self, recv):
        if recv['come_from'] == 'HB':
            db = DBActiveRec(cursorclass=pymysql.cursors.DictCursor)
            try:
                ...
                if len(value['rows']) == 0:
                    ...
                    return
                device = value['rows'][0]
                check_host = db.find_by_host_ip(recv['host_ip'], 'ops_hosts')
                log.logger.info(str(device))
                if isinstance(check_host, list) and check_host[0]:
                    exists = True
                else:
                    log.logger.error(db.error_msg)
                    exists = False
                ...
                if exists is True:
                    ...
                    db.update('ops_hosts', device, {'host_ip': recv['host_ip']}, 1)
                    if db.error_msg is not None:
                        log.logger.error(db.error_msg)
                if exists is False:
                    fields = db.get_fields('ops_hosts')
                    columns = [col['Field'] for col in fields]
                    for k, v in items:
                        if k not in columns or v is None:
                            device.pop(k)
                    device['host_ip'] = recv['host_ip']
                    log.logger.info(str(device))
                    db.insert('ops_hosts', device)
                    if db.error_msg is not None:
                        log.logger.error(db.error_msg)
                return
        ...
        if recv['come_from'] == 'task':
            ...
            load = dict()
            load.update(recv)
            load['come_from'] = 'master'
            load['auth_key'] = auth_key
