...
class DBActiveRec(DB):
    ...
    def __init__(self, cursorclass=pymysql.cursors.Cursor, conn=None):
        self._reset_select()
        self._reset_write()
        super(DBActiveRec, self).__init__(cursorclass=cursorclass, conn=conn)

    def connect(self):
        self._reset_select()
        self._reset_write()
        return super(DBActiveRec, self).connect()
    ...
    def get(self, table='', limit=None, offset=''):
        ...
        result = self.exec_sql(sql, self._auto_log)
        self._reset_select()
        return result
    ...
    def insert(self, table='', _set=None):
        ...
        self._reset_write()
        return self.exec_sql(sql, self._auto_log)
...
if __name__ == '__main__':
    db = DBActiveRec()
    res = db.select('*').from_('z_task').where({'task_uuid': 'd291425d-8e9f-e90f-8298-0c96d50c7a87', 'task_type':3}).get()
    print res
    res = db.select(['task_id', 'task_type', 'task_ip']).where(
