...
class JobHandler(multiprocessing.Process):
    ...
    def run(self):
        try:
            ...
            self.job_id = data_info.get('job_id')
            self.job_type = int(data_info.get('job_type', -1))
            release_type = data_info['release_type']
            domain = data_info.get('domain')
            url = data_info.get('url', '').strip()
            # 0 正式； 1 灰度； 2 测试
            environment = int(data_info.get('environment', 0))
            # 1 并行；0 串行
            parallel = int(data_info.get('parallel', 1))
        ...
        if release_type in ['jetty', 'task', 'static', 'kive']:
            ...
            dist_dir = ''
        elif release_type == 'do_iptables':
            action = data_info.get('action', None)
            if action is None:
                log.logger.error("do_iptables need action up|down")
                return
            load = dict(
                come_from='task',
                job_id=self.job_id,
        ...
        for ip in ip_list:
            if not ip: continue
            extra_load['server_ip'] = ip
            extra_load['task_id'] = "{0:%Y%m%d%H%M%S%f}".format(datetime.datetime.now())
            load.update(extra_load)
            self.tasks.append(
                copy.deepcopy(load)
            )
        ...
        if parallel == 0:
            try:
                for task_info in self.tasks:
                    ...
                    # check
                    _select = ('status', 'ret_code')
                    _where = {
                        "job_id": task_info.get('job_id'),
                        "task_type": task_info.get('task_type') or '0',
                        "task_id": task_info.get('task_id')
                    }
                    while 1:
                        self.db._auto_log = False
                        result = self.db.select(_select).where(_where).get('ops_task', 1)
                        if result is False:
                            raise Exception("task error, %s" % self.db.error_msg or "not exists")
                        if result[0].get('status') == 0:
                            time.sleep(0.1)
                            continue
                        elif result[0].get('status') == 1:
                            break
                        elif result[0].get('status') == 2:
                            err = "task failed, stop all the other tasks"
                            sqlstr = """UPDATE `ops_task`
                                        SET `task_proc`=CONCAT(IFNULL(`task_proc`, ''),'%s %s\n'), `modify_timestamp`='%s'
                                        WHERE `task_id`='%s';
                            """ % (get_cur_datetime(), err, get_cur_datetime(),task_info.get('task_id'))
                            self.db.exec_sql(sqlstr)
                            raise Exception(err)
...
class TaskHandler(object):
    ...
    def create_task(self, task_info):
        ...
        sqlstr = """SELECT *
                    FROM `ops_task`
                    WHERE `server_ip`='%(server_ip)s' AND `status`=0
                    AND UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(`create_timestamp`) < 60*60
