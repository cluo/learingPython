...
function install_zops(){
	...
	sed -i "/subsys/d" /data/ops/zops/agent/bin/zops_agent.sh
    mkdir -p /data/ops
    ...
    cd ~
    sed -i '/bin\/zops_keep_watch.sh/d' /var/spool/cron/crontabs/root
    echo '*  *  *  *  * /bin/bash /data/ops/zops/agent/bin/zops_keep_watch.sh >> /var/log/zops_keep_watch 2>&1' >> /var/spool/cron/crontabs/root
    return 0
}
